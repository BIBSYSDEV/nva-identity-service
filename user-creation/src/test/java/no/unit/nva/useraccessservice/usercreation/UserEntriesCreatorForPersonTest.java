...
class UserEntriesCreatorForPersonTest {
  ...
  // userCreator.createUsers(personInformation, customers));
  //    }
  ...
  @Test
  @DisplayName(
      "should create User for Institution when the Person exists in the Person-Registry,"
  ...
  void
      shouldCreateUserForInstWhenPersonExistsAndInstIsNvaCustomerAndPersonHasSingleActiveAffiliation() {
  ...
  @Test
  @DisplayName(
      "should not create User for Institution when the Person exists in the Person-Registry,"
          + "and they have an Affiliation with the Institution "
          + "and the Affiliation is inactive"
          + "and the the Institution is an NVA Customer"
          + "and the Person has no other Affiliations active or inactive")
  void
      shouldNotCreateUserForInstWhenPersonExistsAndInstIsNvaCustomerAndPersonHasSingleInactiveAffiliation() {
  ...
  @Test
  @DisplayName(
      " Given that a Person exists in the Person-Registry,And they have some active Affiliations in"
          + " some OrganizationsAnd the  have some inactive Affiliations is some other"
          + " OrganizationsAnd the parent Institutions are all NVA customersThen it should create a"
          + " User for each Institution where the Person has an active affiliation with")
  ...
  @Test
  @DisplayName(
      " Given that a Person exists in the Person-Registry,"
  ...
  void shouldNotOverwriteUsernameOfExistingUsers() {
    ...
    var userCreationContext = new UserCreationContext(person, customers);
    var existingUser =
        scenarios.createUsersForAllActiveAffiliations(personNin, identityService).stream()
            .collect(SingletonCollector.collect());
    var actualUser =
        userCreator.createUsers(userCreationContext).stream().collect(SingletonCollector.collect());
  ...
  void shouldUpdateLegacyFeideUserWithNecessaryDetailsWhenSuchUserExists() {
    ...
    var existingUser =
        scenarios
            .createLegacyUsersForAllActiveAffiliations(personNin, feideIdentifier, identityService)
            .stream()
            .collect(SingletonCollector.collect());
    var person = personRegistry.fetchPersonByNin(nin).orElseThrow();
    var customers = fetchCustomersWithActiveAffiliations(person.getAffiliations());
    ...
    var actualUser =
        userCreator.createUsers(userCreationContext).stream().collect(SingletonCollector.collect());
    var expectedUser =
        constructExpectedUpdatedUserForLegacyFeideUser(personNin, feideIdentifier, existingUser);

    assertThat(actualUser, samePropertyValuesAs(expectedUser));
  }

  private UserDto constructExpectedUpdatedUserForLegacyFeideUser(
      String person, String feideIdentifier, UserDto existingUser) {

    var expectedInstitutionCristinId =
        scenarios.getCristinUriForInstitutionAffiliations(person, true).stream()
            .collect(SingletonCollector.collect());
  ...
  @Test
  void
      shouldCreateUserForSpecificCustomerInstitutionWhenPersonHasActiveAffiliationWithCustomerInstitution() {
    ...
    var userCreationContext =
        new UserCreationContext(person, Collections.singleton(selectedCustomer));
