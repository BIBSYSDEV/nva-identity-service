...
class CreateUserHandlerTest extends HandlerTest {
  ...
  public void init(WireMockRuntimeInfo wireMockRuntimeInfo)
    ...
    var mockPersonRegistry =
        new MockPersonRegistry(
            cristinUsername, cristinPassword, wiremockUri, defaultRequestHeaders);

    this.scenarios =
        new AuthenticationScenarios(mockPersonRegistry, customerService, identityService);
    ...
    handler =
        new CreateUserHandler(identityService, customerService, personRegistry, new Environment());
  ...
  @Test
  void shouldCreateUserWithRequestedRolesAndViewingScopeWhenCreatedWithNin() throws IOException {
    ...
    var requestBody =
        sampleRequestForExistingPersonCustomerAndRolesWithoutCristinId(
            person, someCustomer.getId(), providedViewingScope);
    ...
    var actualUser =
        identityService.listUsers(requestBody.customerId()).stream()
  ...
  private CreateUserRequest sampleRequestForExistingPersonCustomerAndRolesWithoutCristinId(
      MockedPersonData person, URI customerId, ViewingScope viewingScope) {
    return new CreateUserRequest(person.nin(), null, customerId, randomRoles(), viewingScope);
  }

  private Set<RoleDto> randomRoles() {
    var role =
        RoleDto.newBuilder().withRoleName(randomRoleNameButNot(RoleName.APPLICATION_ADMIN)).build();
  ...
  private InputStream createRequest(
      CreateUserRequest requestBody, CustomerDto customer, AccessRight... accessRights)
  ...
  void shouldCreateUserWithRequestedRolesAndViewingScopeWhenCreatedWithCristinId()
    ...
    var providedViewingScope = ViewingScope.defaultViewingScope(randomCristinOrganization());
    var requestBody =
        sampleRequestForExistingPersonCustomerAndRolesWithoutNin(
            person, someCustomer.getId(), providedViewingScope);
    var request = createRequest(requestBody, someCustomer, MANAGE_OWN_AFFILIATION);
    var response = sendRequest(request, UserDto.class);
  ...
  private CreateUserRequest sampleRequestForExistingPersonCustomerAndRolesWithoutNin(
      MockedPersonData person, URI customerId, ViewingScope viewingScope) {
    return new CreateUserRequest(
        null, person.cristinIdentifier(), customerId, randomRoles(), viewingScope);
  }

  @Test
  void
      shouldCreateUserWithRequestedRolesWhenInputContainsNationalIdNumberInstitutionIdAndSetOfRoles()
          throws IOException {
    ...
    var requestBody =
        sampleRequestForExistingPersonCustomerAndRoles(
            person,
            customer.getCristinId(),
    ...
    var actualUser =
        identityService.listUsers(customer.getId()).stream().collect(SingletonCollector.collect());
  ...
  private CreateUserRequest sampleRequestForExistingPersonCustomerAndRoles(
      MockedPersonData person, URI customerId, ViewingScope viewingScope) {
    return new CreateUserRequest(
        person.nin(), person.cristinIdentifier(), customerId, randomRoles(), viewingScope);
  }

  @Test
  void shouldReturnOkAndReturnExistingUserWithStatusCode200WhenTryingToCreateExistingUser()
      throws IOException {
    var person = scenarios.personWithExactlyOneActiveEmployment();
    var customer = fetchSomeCustomerForThePerson(person);
    ...
    var requestBody =
        sampleRequestForExistingPersonCustomerAndRoles(
            person, customer.getId(), ViewingScope.defaultViewingScope(organizationId));
  ...
  private InputStream createRequestWithoutAccessRights(CreateUserRequest requestBody)
      throws JsonProcessingException {
  ...
  @Test
  void
      shouldDenyAccessToUsersThatDoNotHaveTheRightToAddAnyUserAndTheyAreTryingToAddUsersForAnotherInstitution()
          throws IOException {
    var customerIdentifier = UUID.randomUUID();
    var customerId =
        UriWrapper.fromUri(randomUri()).addChild(customerIdentifier.toString()).getUri();
    var requestBody =
        sampleRequestForExistingPersonCustomerAndRoles(
            randomPerson(),
            customerId,
  ...
  @Test
  void shouldAllowAccessToBackendServices() throws IOException {
    var person = scenarios.personWithOneActiveAndOneInactiveEmploymentInDifferentInstitutions();
    var customer = fetchSomeCustomerForThePerson(person);
    var requestBody =
        sampleRequestForExistingPersonCustomerAndRoles(
  ...
  @Test
  void shouldReturnExistingUserAndNotOverwriteItWhenAttemptingToCreateAlreadyExistingUser()
      throws IOException {
    var person = scenarios.personWithExactlyOneActiveEmployment();
    var customer = fetchSomeCustomerForThePerson(person);
    var requestBody =
  ...
  void shouldReturnConflictErrorWhenPersonHasNoActiveAffiliations() throws IOException {
    ...
    var requestBody =
        sampleRequestForExistingPersonCustomerAndRoles(
            person, customer.getId(), ViewingScope.defaultViewingScope(customer.getCristinId()));
    var request = createRequest(requestBody, customer, MANAGE_OWN_AFFILIATION);
    ...
    assertThat(
        message, matchesPattern(".*Person (.*) has no active affiliations with an institution.*"));
  }
  ...
  void shouldReturnConflictErrorWhenPersonHasActiveAffiliationsButNotWithAnExistingCustomer()
    ...
    assertThat(
        message,
        matchesPattern(
            ".*Person (.*) has no active affiliations with an Institution that"
                + " is an NVA customer.*"));
  }
  ...
  void shouldReturnConflictErrorWhenPersonIsNotRegisteredInPersonRegistry() throws IOException {
    ...
    var message = response.getBodyObject(Problem.class).getDetail();
    assertThat(message, matchesPattern(".*Person (.*) is not registered in the Person Registry.*"));
  }
  ...
  void shouldReturnBadGatewayWhenSomethingUnexpectedError() throws IOException {
    ...
    handler =
        new CreateUserHandler(
            mockedIdentityService, customerService, personRegistry, new Environment());
    handler.handleRequest(request, outputStream, context);
    ...
    var message = response.getBodyObject(Problem.class).getDetail();
    assertThat(message, is(equalTo("Something went wrong, contact application administrator!")));
  }
