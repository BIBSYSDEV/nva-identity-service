...
public class CustomerTableDynamodbStreamToEventBridgeHandlerTest {
  ...
  private static final Set<PublicationInstanceTypes> DEFAULT_SCOPE =
      Set.of(
          DEGREE_PHD,
          DEGREE_MASTER,
          DEGREE_BACHELOR,
  ...
  void beforeEach() {
    ...
    this.handler =
        new CustomerTableDynamodbStreamToEventBridgeHandler(environment, eventBridgeClient);
  ...
  private ChannelClaimContext generateContext() {
    ...
    return new ChannelClaimContext(
        channelClaimId,
        channelIdentifier,
        channelId,
        customerIdentifier,
  ...
  void shouldThrowWhenEventBridgeClientFailsWithHttpStatus() {
    ...
    handler =
        new CustomerTableDynamodbStreamToEventBridgeHandler(
            environment, new FailingEventBridgeClient(true));
  ...
  void shouldThrowWhenEventBridgeClientFailsOnIndividualEntries() {
    ...
    handler =
        new CustomerTableDynamodbStreamToEventBridgeHandler(
            environment, new FailingEventBridgeClient(false));
  ...
  void shouldEmitEventForNewChannelClaimAddedOnCreationOfNewCustomer() {
    ...
    assertThat(
        ...
        containsInAnyOrder(
            expectedEvent(
                ResourceUpdateEvent.Action.ADDED,
  ...
  void shouldEmitEventForNewChannelClaimAddedOnCustomerUpdate() {
    var event = customerUpdateEvent(channelClaimContext, DEFAULT_SCOPE, OWNER_ONLY, OWNER_ONLY);
    ...
    assertThat(
        ...
        containsInAnyOrder(
            expectedEvent(
                ResourceUpdateEvent.Action.UPDATED,
                channelClaimContext.channelClaimId,
                channelClaimContext.customerId,
                channelClaimContext.organizationId,
                channelClaimContext.channelId,
  ...
  void shouldEmitEventForChannelClaimDeletedOnCustomerUpdate() {
    ...
    assertThat(
        ...
        containsInAnyOrder(
            expectedEvent(
                ResourceUpdateEvent.Action.REMOVED,
                channelClaimContext.channelClaimId,
                channelClaimContext.customerId,
                channelClaimContext.organizationId,
                channelClaimContext.channelId,
  ...
  void shouldNotFailWhenConsumingEmptyStringInDynamoDbAttribute(String value)
      throws JsonProcessingException {
    var jsonBody = dynamoDbRecordWithStringValue(value);
    var dynamodbStreamRecord =
        JsonUtils.dtoObjectMapper.readValue(jsonBody, DynamodbStreamRecord.class);
    var event = new DynamodbEvent();
  ...
  private URI randomOrganizationId() {
    var identifier =
        new Random().ints(4, 1, 9).mapToObj(String::valueOf).collect(Collectors.joining("."));
    var organizationId = "https://localhost/cristin/organization/" + identifier;
  ...
  private DynamodbEvent customerUpdateEvent(
      ChannelClaimContext context,
  ...
  private DynamodbEvent customerUpdateEvent(ChannelClaimContext context) {
    ...
    streamRecord.setOldImage(
        customerAttributeMap(
            ...
            OWNER_ONLY));
    streamRecord.setNewImage(
        customerAttributeMap(context.customerIdentifier, context.organizationId));
  ...
  private ResourceUpdateEvent<ChannelClaim> expectedEvent(
    ...
    var constraint = new Constraints(scope, publishingPolicy, editingPolicy);
    var channelClaim =
        new ChannelClaim(channelClaimId, channelId, customerId, organizationId, constraint);
    return new ResourceUpdateEvent<>(action, "ChannelClaim", channelClaim);
  }

  private URI channelClaimId(UUID identifier) {
    var channelClaimId = "https://localhost/customer/channel-claim/" + identifier;
    return URI.create(channelClaimId);
  }

  private URI channelId(UUID identifier) {
    var channelId = "https://localhost/publication-channels-v2/serial-publication/" + identifier;
    return URI.create(channelId);
  }

  private URI customerId(UUID identifier) {
    var customerId = "https://localhost/customer/" + identifier;
    return URI.create(customerId);
  }

  private Map<String, AttributeValue> customerAttributeMap(
      UUID identifier,
      URI organizationId,
      URI channelId,
      Set<PublicationInstanceTypes> scope,
    ...
    return Map.of(
        "identifier",
        new AttributeValue().withS(identifier.toString()),
        "cristinId",
        new AttributeValue().withS(organizationId.toString()),
        "rightsRetentionStrategy",
        new AttributeValue().withM(rightsRetentionStrategyArraibuteMap()),
        "channelClaims",
        new AttributeValue()
            .withL(
                new AttributeValue()
                    .withM(
                        channelClaimAttributeMap(
                            channelId, scope, publishingPolicy, editingPolicy))));
  }
  ...
  private Map<String, AttributeValue> rightsRetentionStrategyArraibuteMap() {
    return Map.of("type", new AttributeValue().withS("NullRightsRetentionStrategy"));
  }

  private Map<String, AttributeValue> channelClaimAttributeMap(
      URI channelId,
      Set<PublicationInstanceTypes> scope,
      ChannelConstraintPolicy publishingPolicy,
      ChannelConstraintPolicy editingPolicy) {
    return Map.of(
        ...
        "constraint",
            new AttributeValue()
                .withM(constraintAttributeMap(scope, publishingPolicy, editingPolicy)));
  }

  private Map<String, AttributeValue> constraintAttributeMap(
      Set<PublicationInstanceTypes> scope,
      ChannelConstraintPolicy publishingPolicy,
      ChannelConstraintPolicy editingPolicy) {
    var serializedScope =
