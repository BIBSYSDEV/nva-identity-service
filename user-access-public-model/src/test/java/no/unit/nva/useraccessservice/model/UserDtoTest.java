...
class UserDtoTest extends DtoTest {
  ...
  void userDtoShouldContainEmptyViewingScopeWhenViewingScopeIsNotProvided()
    ...
    var viewingScope =
        toMap(JsonUtils.dtoObjectMapper.writeValueAsString(json.get("viewingScope")));
  ...
  void getAccessRightsReturnsAccessRightsWithoutDuplicates() {
    final UserDto user =
        createUserWithRoleWithoutInstitution(sampleRoles.iterator().next().getRoleName());
  ...
  void getAccessRightsReturnsAllAccessRightsContainedInTheUsersRoles() {
    ...
    assertThat(
        ...
        containsInAnyOrder(expectedAccessRights.toArray(AccessRight[]::new)));
  }
  ...
  private RoleDto sampleRole(AccessRight accessRight, RoleName someRolename)
    ...
    return RoleDto.newBuilder().withRoleName(someRolename).withAccessRights(accessRights).build();
  }
  ...
  void builderReturnsUserDtoWhenIRolesIsEmpty() {
    UserDto user =
        UserDto.newBuilder().withUsername(SOME_USERNAME).withInstitution(SOME_INSTITUTION).build();
    assertThat(user.getUsername(), is(equalTo(SOME_USERNAME)));
  ...
  void userDtoContainsCristinUnitsToBeIncludedToCuratorsView() throws BadRequestException {
    ...
    ViewingScope viewingScope =
        ViewingScope.create(Set.of(cristinUnitIncludedInDefaultCuratorsView), null);
    UserDto userDto =
        UserDto.newBuilder().withUsername(randomString()).withViewingScope(viewingScope).build();
    ViewingScope actualViewingScope = userDto.getViewingScope();
    assertThat(
        actualViewingScope.getIncludedUnits(), contains(cristinUnitIncludedInDefaultCuratorsView));
  }
  ...
  void userDtoContainsCristinUnitsToBeExcludedToCuratorsView() throws BadRequestException {
    ...
    ViewingScope viewingScope =
        ViewingScope.create(
            Set.of(cristinUnitIncludedInDefaultCuratorsView),
    ...
    assertThat(
        actualViewingScope.getIncludedUnits(), contains(cristinUnitIncludedInDefaultCuratorsView));
    assertThat(
        actualViewingScope.getExcludedUnits(),
        contains(cristinUnitExcludedFromDefaultCuratorsView));
  }
  ...
  private void assertThatThereIsARoleClaimForEachRole(UserDto user, Set<String> expectedRolenames) {
    var actualRolenamesInClaims =
        user.generateRoleClaims()
  ...
  @Test
  void
      shouldReturnViewingScopeSetToTopLevelOrgWhenScopeIsNotExplicitlyDefinedAndUserIsConnectedToSomeTopLevelOrg() {
  ...
  @Test
  void
      shouldReturnViewingScopeWithEmptySetsWhenScopeIsNotExplicitlyDefinedAndUserIsNotConnectedToSomeTopLevelOrg() {
