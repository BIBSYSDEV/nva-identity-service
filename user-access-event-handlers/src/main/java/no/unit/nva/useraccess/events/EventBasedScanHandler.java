...
public class EventBasedScanHandler extends EventHandler<ScanDatabaseRequestV2, Void> {
  ...
  @JacocoGenerated
  private static UserMigrationServiceImpl defaultMigrationService(DynamoDbClient dynamoDBClient) {
  ...
  protected Void processInput(
      ScanDatabaseRequestV2 scanDatabaseRequest,
  ...
  private void emitNexScanRequestIfThereAreMoreResults(
      UserScanResult scanResult, ScanDatabaseRequestV2 inputRequest, Context context) {
  ...
  private void emitNextScanRequest(
      UserScanResult scanResult, ScanDatabaseRequestV2 inputRequest, Context context) {
  ...
  private PutEventsRequest creteEventForNextPageScan(
      ScanDatabaseRequestV2 inputScanRequest, UserScanResult scanResult, Context context) {
  ...
  private PutEventsRequestEntry createNewEventEntry(
      Context context, ScanDatabaseRequestV2 scanRequest) {
    return scanRequest.createNewEventEntry(
        EventsConfig.EVENT_BUS,
        EventsConfig.SCAN_REQUEST_EVENTS_DETAIL_TYPE,
        context.getInvokedFunctionArn());
  }

  private ScanDatabaseRequestV2 createNextScanRequest(
      ScanDatabaseRequestV2 input, UserScanResult scanResult) {
