...
public class StartBatchScan implements RequestStreamHandler {
  ...
  private ScanDatabaseRequestV2 createEventAsExpectedByEventListener(ScanDatabaseRequestV2 input) {
    return new ScanDatabaseRequestV2(
        IDENTITY_SERVICE_BATCH_SCAN_EVENT_TOPIC, input.getPageSize(), START_FROM_BEGINNING);
  }
  ...
  private PutEventsRequestEntry createNewEventEntry(
      Context context, ScanDatabaseRequestV2 request) {
    return request.createNewEventEntry(
        EVENT_BUS, SCAN_REQUEST_EVENTS_DETAIL_TYPE, context.getInvokedFunctionArn());
  }
}
