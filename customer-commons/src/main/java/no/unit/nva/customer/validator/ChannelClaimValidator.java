...
public final class ChannelClaimValidator {
  ...
  private static final String PROVIDED_CONSTRAINT_IS_NOT_ALLOWED =
      "Provided constraint is not allowed";
  private static final Environment ENVIRONMENT = new Environment();
  private static final String PUBLICATION_CHANNEL_PATH =
      ENVIRONMENT.readEnv("PUBLICATION_CHANNEL_PATH");
  ...
  private static final Set<PublicationInstanceTypes> DEGREES =
      EnumSet.of(
          DEGREE_BACHELOR,
          DEGREE_MASTER,
          DEGREE_PHD,
  ...
  private static boolean isValidPath(URI channel) {
    ...
    return isChannelPath(pathElements[0])
        && isChannelType(pathElements[1])
        && isUuid(pathElements[2]);
  }
