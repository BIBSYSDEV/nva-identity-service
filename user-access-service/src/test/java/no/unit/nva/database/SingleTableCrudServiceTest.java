...
public class SingleTableCrudServiceTest {
  ...
  static void initialize() {
    ...
    new SingleTableTemplateCreator(client).createTable(tableName);
  ...
  void shouldPersistPreferencesAndLicense() throws NotFoundException {
    ...
    var fetchedTermsConditions = persistedTermsConditions.fetch(singleTableCrudService);
  ...
  void shouldUpdateTermsConditions() throws NotFoundException {
    ...
    var termsConditions =
        TermsConditions.builder().id(userIdentifier).build().fetch(singleTableCrudService);
  ...
  void shouldThrowExceptionWhenFetchingNonExistentTermsConditions() {
    assertThrows(
        NotFoundException.class,
        () -> TermsConditions.builder().id(randomUri()).build().fetch(singleTableCrudService));
  }

  @Test
  void shouldThrowExceptionWhenDeletingNonExistentTermsConditions() {
    var dao = TermsConditions.builder().id(randomUri()).build();
    assertThrows(NotFoundException.class, () -> singleTableCrudService.delete(dao));
  }
}
