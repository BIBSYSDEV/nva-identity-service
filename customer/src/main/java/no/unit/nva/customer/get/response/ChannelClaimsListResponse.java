...
public record ChannelClaimsListResponse(Collection<ChannelClaimResponse> channelClaims) {
  ...
  public static ChannelClaimsListResponse fromChannelClaims(
      Collection<ChannelClaimWithClaimer> channelClaims) {
    return channelClaims.stream()
        .map(ChannelClaimsListResponse::getChannelClaimResponse)
        .collect(collectingAndThen(Collectors.toList(), ChannelClaimsListResponse::new));
  }

  private static ChannelClaimResponse getChannelClaimResponse(
      ChannelClaimWithClaimer channelClaimWithClaimer) {
    var claimedBy =
        new CustomerResponse(
            channelClaimWithClaimer.customerId(), channelClaimWithClaimer.cristinId());
