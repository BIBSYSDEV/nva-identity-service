...
public class ListByInstitutionHandler extends ApiGatewayHandler<Void, UserList> {
  ...
  public static final String MISSING_QUERY_PARAMETER_ERROR =
      "Institution Id query parameter is not a URI. "
  ...
  @Override
  protected UserList processInput(Void body, RequestInfo input, Context context)
    ...
    var users =
        databaseService.listUsers(institutionId).stream()
  ...
  private URI extractInstitutionIdFromRequest(RequestInfo requestInfo) throws BadRequestException {
    return requestInfo
        .getQueryParameterOpt(INSTITUTION_ID_QUERY_PARAMETER)
        .map(URI::create)
  ...
  private boolean likeUserOrEmpty(UserDto userDto, Optional<String> userName) {
    return userName.map(s -> userDto.getUsername().contains(s)).orElse(true);
  }
