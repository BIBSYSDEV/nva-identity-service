...
public class CognitoService {
  ...
  public static final String EXTERNAL_SCOPE_IDENTIFIER =
      new Environment().readEnv("EXTERNAL_SCOPE_IDENTIFIER");
  private static final String EXTERNAL_USER_POOL_ID =
      new Environment().readEnv("EXTERNAL_USER_POOL_ID");
  ...
  public CreateUserPoolClientResponse createUserPoolClient(
      String appClientName, List<String> scopes) throws BadRequestException {
    return attempt(
            () ->
                client.createUserPoolClient(buildCreateCognitoClientRequest(appClientName, scopes)))
        .orElseThrow(fail -> handleUserPoolCreationFailure(fail.getException(), scopes));
  }

  private CreateUserPoolClientRequest buildCreateCognitoClientRequest(
      String appClientName, Collection<String> scopes) {
  ...
  private BadRequestException handleUserPoolCreationFailure(
      Exception exception, Collection<String> scopes) {
  ...
  private Collection<String> getUnknownScopes(
      Collection<String> knownScopes, Collection<String> usedScopes) {
