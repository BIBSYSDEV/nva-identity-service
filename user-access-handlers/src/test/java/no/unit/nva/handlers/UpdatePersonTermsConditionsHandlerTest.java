...
class UpdatePersonTermsConditionsHandlerTest extends HandlerTest {
  ...
  void setup() {
    ...
    response = TermsConditionsResponse.builder().withTermsConditionsUri(randomUri()).build();
  }
  ...
  void shouldUpdateCognitoWithLatestTermsSelection() throws IOException, NotFoundException {
    ...
    ArgumentCaptor<AdminUpdateUserAttributesRequest> captor =
        forClass(AdminUpdateUserAttributesRequest.class);
    ...
    assertTrue(
        capturedAttributes.stream()
            .anyMatch(attribute -> attribute.name().equals(CognitoClaims.CUSTOMER_ACCEPTED_TERMS)));

    assertTrue(
        capturedAttributes.stream()
            .anyMatch(attribute -> attribute.name().equals(CognitoClaims.CURRENT_TERMS)));
  }
  ...
  private ObjectNode getRequestContext() {
    return objectMapperWithEmpty.convertValue(
        Map.of("path", "/terms-and-conditions/", "domainName", "SAMPLE_DOMAIN_NAME"),
        ObjectNode.class);
  }
}
