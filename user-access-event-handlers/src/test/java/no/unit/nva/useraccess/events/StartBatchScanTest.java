...
class StartBatchScanTest {
  ...
  @Test
  void
      shouldSendEventContainingTheEventTypeForBatchScanningAndNullStartingPointWhenInputIsAnEmptyObject()
          throws IOException {
    batchScanner.handleRequest(emptyInput(), outputStream, CONTEXT);
    var emittedEventBody = extractEventBody();
    var expectedEmittedEventBody =
        new ScanDatabaseRequestV2(
            IDENTITY_SERVICE_BATCH_SCAN_EVENT_TOPIC, ScanDatabaseRequestV2.DEFAULT_PAGE_SIZE, null);
  ...
  @Test
  void shouldSendEventToEventBusDefinedInTheEnvironment() throws IOException {
    batchScanner.handleRequest(emptyInput(), outputStream, CONTEXT);
