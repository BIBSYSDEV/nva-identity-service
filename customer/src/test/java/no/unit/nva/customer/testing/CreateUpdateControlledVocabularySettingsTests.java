...
public abstract class CreateUpdateControlledVocabularySettingsTests
    extends LocalCustomerServiceDatabase {
  ...
  protected <T> InputStream createRequest(
      UUID customerIdentifier, T expectedBody, MediaType acceptedMediaType)
  ...
  protected ExpectedBodyActualResponseTuple sendRequestWithAccessRight(
      UUID uuid, AccessRight accessRight) throws IOException {
    VocabularyList expectedBody = createRandomVocabularyList();
  ...
  protected <T> InputStream createRequest(
      UUID customerIdentifier, T expectedBody, AccessRight accessRight)
      throws JsonProcessingException {
    return new HandlerRequestBuilder<T>(dtoObjectMapper)
        .withPathParameters(identifierToPathParameter(customerIdentifier))
  ...
  protected void assertThatExistingUserHasEmptyVocabularySettings() throws IOException {
    GetControlledVocabularyHandler getHandler =
        new GetControlledVocabularyHandler(customerService, new Environment());
  ...
  protected <T> GatewayResponse<T> sendRequest(
      ControlledVocabularyHandler<?, ?> getHandler, InputStream getRequest, Class<T> responseType)
      throws IOException {
