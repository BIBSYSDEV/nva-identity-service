...
class UpdateCustomerDoiHandlerTest {
  ...
  public void beforeEach() throws NotFoundException, InputException {
    ...
    handler =
        new UpdateCustomerDoiHandler(
            customerServiceMock, mock(SecretsWriter.class), secretsReaderMock);
    ...
    when(customerServiceMock.getCustomer(any(UUID.class))).thenReturn(existingCustomer);

    when(customerServiceMock.updateCustomer(any(UUID.class), any(CustomerDto.class)))
        .thenReturn(existingCustomer);

    when(secretsReaderMock.fetchSecret(any(), any())).thenReturn(secretDaoArray);
  }
  ...
  /** Task. <a href="https://unit.atlassian.net/browse/NP-27814">NP-27814</a> */
  @Test
  void shouldUpdateExistingSecretWhenIdentifierFoundInSecrets()
    ...
    var response =
        sendRequest(
            existingCustomer.getIdentifier(),
  ...
  private <T> GatewayResponse<T> sendRequest(UUID identifier, T body, Class<T> responseType)
      throws IOException {
  ...
  private <T> InputStream createRequest(UUID identifier, T body) throws JsonProcessingException {
  ...
  private String doiAgentToJson(DoiAgentDto doiAgent, String secretPassword) {
    return new DoiAgentDto(doiAgent).addId(doiAgent.getId()).addPassword(secretPassword).toString();
  }

  /**
   * Task. <a href="https://unit.atlassian.net/browse/NP-27812">NP-27812</a> This also tests that
   * secret is persisted with the identifier in the request.
   */
  @Test
  void shouldInsertSecretWhenIdentifierNotFoundInSecrets() throws ApiGatewayException, IOException {
  ...
  void shouldReturnSecretWithEmptyUserNameWhenPersistingEmptyUserName()
    ...
    var response =
        sendRequest(existingCustomer.getIdentifier(), existingDoiAgent.toString(), String.class);
    var doiAgentResponse = DoiAgentDto.fromJson(response.getBody());
  ...
  void failUpdateRequestReturnsBadRequestWhenARequestWithAInvalidIdentifier()
      throws ApiGatewayException, IOException {
    when(customerServiceMock.getCustomer(any(UUID.class))).thenThrow(NotFoundException.class);

    var response = sendRequest(createRequestWithInvalidUuid(null), Problem.class);
    assertThat(response.getStatusCode(), is(equalTo(HttpURLConnection.HTTP_BAD_REQUEST)));
  }

  private <T> InputStream createRequestWithInvalidUuid(T body) throws JsonProcessingException {
    return new HandlerRequestBuilder<T>(dtoObjectMapper)
        .withHeaders(getRequestHeaders())
