...
public class CustomerTableDynamodbStreamToEventBridgeHandler
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(CustomerTableDynamodbStreamToEventBridgeHandler.class);
  private static final String ENV_KEY_EVENT_BUS_NAME = "EVENT_BUS_NAME";
  private static final String DETAIL_TYPE_RESOURCE_UPDATE_CHANNEL_CLAIM =
  ...
  public CustomerTableDynamodbStreamToEventBridgeHandler(
      final Environment environment, final EventBridgeClient eventBridgeClient) {
    this.customerResourceUpdateEventsProducer =
        new DefaultCustomerResourceUpdateEventsProducer(new JacksonAttributeValueConverter());
    this.eventBridgeClient = eventBridgeClient;
    this.environment = environment;
  }

  @Override
  public List<ResourceUpdateEvent<ChannelClaim>> handleRequest(
      DynamodbEvent dynamodbEvent, Context context) {
    var events =
    ...
    var eventsAsJson =
        attempt(() -> JsonUtils.dtoObjectMapper.writeValueAsString(events)).orElseThrow();
    logger.info("Derived the following events from dynamodb stream: {}", eventsAsJson);

    var eventEmitter =
        new EventBridgeClientResourceUpdatedEventEmitter(
            eventBridgeClient, environment.readEnv(ENV_KEY_EVENT_BUS_NAME), context);
    if (events.isEmpty()) {
