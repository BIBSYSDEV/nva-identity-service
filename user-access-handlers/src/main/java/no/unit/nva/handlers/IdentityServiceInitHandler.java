...
public class IdentityServiceInitHandler extends ApiGatewayHandler<Void, RoleList> {
  ...
  public static final URI SIKT_CRISTIN_ID =
      URI.create(new Environment().readEnv("SIKT_CRISTIN_ID"));
  ...
  private void addSiktBackendClientUser(CustomerDto sikt) {
    ...
    // The Cognito user pool client credentials have already been set up in Cognito. However, the
    // client still
    ...
    attempt(
            () -> {
              ...
              return identityService.getClient(
                  ClientDto.newBuilder().withClientId(backendClientId).build());
            })
        .or(
            () -> {
