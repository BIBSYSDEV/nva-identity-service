...
public class CustomerDto {
  ...
  public CustomerDto addChannelClaim(ChannelClaimDto channelClaim) {
    if (getChannelClaims().stream()
        .anyMatch(item -> item.channel().equals(channelClaim.channel()))) {
      return this;
    }
  ...
  public void setCristinId(URI cristinId) {
    this.cristinId =
        nonNull(cristinId) && StringUtils.isNotBlank(cristinId.toString()) ? cristinId : null;
  }
  ...
  public static final class Builder {
    ...
    private RightsRetentionStrategyDto buildRetentionStrategyDto(
    ...
    public Builder withChannelClaims(Collection<ChannelClaimDto> channelClaims) {
      customerDto.setChannelClaims(
          nonNull(channelClaims) ? channelClaims : Collections.emptyList());
      return this;
    }

    public Builder withAutomaticallyPublishFilesFromScopusImport(
        boolean isAutoPublishScopusImportFiles) {
  ...
  public static class DoiAgentDto implements DoiAgent {
    ...
    public static DoiAgentDto fromJson(String json) throws BadRequestException {
      return attempt(() -> JsonConfig.readValue(json, DoiAgentDto.class))
          .orElseThrow(
              fail ->
                  new BadRequestException("Could not parse input:" + json, fail.getException()));
    }
    ...
    public DoiAgentDto addIdByIdentifier(UUID identifier) {
      this.id = UriWrapper.fromUri(toId(identifier)).addChild(DOI_AGENT).getUri();
      return this;
    }
