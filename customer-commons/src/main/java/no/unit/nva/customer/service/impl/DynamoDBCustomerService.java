...
public class DynamoDBCustomerService implements CustomerService {
  ...
  public static final String IDENTIFIERS_NOT_EQUAL =
      "Identifier in request parameters '%s' "
  ...
  private static final String DYNAMODB_WARMUP_PROBLEM =
      "There was a problem during describe table to warm up " + "DynamoDB connection";
  private static final String CUSTOMER_ALREADY_EXISTS_ERROR =
      "Customer with Institution ID %s already exists.";
  ...
  public CustomerDto getCustomerByOrgDomain(String orgDomain) throws NotFoundException {
    ...
    return sendQueryToIndex(
        query, BY_ORG_DOMAIN_INDEX_NAME, CustomerDao::getFeideOrganizationDomain);
  }
  ...
  public CustomerDto putCustomer(
      UUID identifier, CustomerDto customer, boolean shouldOverwriteChannelClaims)
  ...
  public CustomerDto getCustomerByCristinId(URI cristinId) throws NotFoundException {
    ...
    return sendQueryToIndex(
        queryObject, BY_CRISTIN_ID_INDEX_NAME, customer -> customer.getCristinId().toString());
  }
  ...
  private void throwIfChannelIsAlreadyClaimed(ChannelClaimDto channelClaim)
  ...
  public Optional<ChannelClaimWithClaimer> getChannelClaim(UUID identifier) {
    return getCustomers().stream()
        .flatMap(DynamoDBCustomerService::toChannelClaimWithClaimer)
        .filter(
            channelClaimWithClaimer ->
                identifier.equals(channelClaimWithClaimer.channelClaimIdentifier()))
        .findFirst();
  }
  ...
  private static ChannelClaimWithClaimer getChannelClaimWithClaimer(
      ChannelClaimDto channelClaim, CustomerDto customer) {
  ...
  private void validateIdentifier(UUID identifier, CustomerDto customer) throws InputException {
    if (!identifier.equals(customer.getIdentifier())) {
      throw new InputException(
          String.format(IDENTIFIERS_NOT_EQUAL, identifier, customer.getIdentifier()), null);
    }
  }
  ...
  private void checkForConflict(URI institutionId) throws ConflictException {
    var customerExists =
        attempt(() -> getCustomerByCristinId(institutionId)).toOptional().isPresent();
    if (customerExists) {
      throw new ConflictException(
          String.format(CUSTOMER_ALREADY_EXISTS_ERROR, institutionId.toString()));
    }
  }

  private CustomerDto sendQueryToIndex(
      CustomerDao queryObject, String indexName, Function<CustomerDao, String> indexPartitionValue)
      throws NotFoundException {
    var queryValue = indexPartitionValue.apply(queryObject);
  ...
  private QueryEnhancedRequest createQuery(
      CustomerDao queryObject, Function<CustomerDao, String> indexPartitionValue) {
