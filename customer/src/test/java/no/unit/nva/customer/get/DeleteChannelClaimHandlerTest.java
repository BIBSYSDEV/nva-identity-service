...
class DeleteChannelClaimHandlerTest {
  ...
  void shouldThrowBadRequestWhenProvidedChannelClaimIdentifierIsNotValid() throws IOException {
    var request =
        createAuthenticatedRequest(randomString(), randomUUID().toString(), MANAGE_CHANNEL_CLAIMS);
  ...
  void shouldThrowInternalServerErrorWhenUnexpectedFailureOccurs()
    ...
    new DeleteChannelClaimHandler(failingCustomerService(), new Environment())
  ...
  @Test
  void shouldReturnNoContentWhenChannelHasBeenSuccessfullyDeletedOnBehalfOfAnotherCustomer()
      throws IOException {
    var request =
        createAuthenticatedRequest(randomUUID(), randomUUID(), randomUUID(), MANAGE_CHANNEL_CLAIMS);
  ...
  private InputStream createAuthenticatedRequest(
      UUID channelClaimIdentifier,
      UUID userAndChannelClaimCustomerIdentifier,
      AccessRight accessRight)
      throws JsonProcessingException {
    return createAuthenticatedRequest(
        channelClaimIdentifier.toString(),
  ...
  private InputStream createAuthenticatedRequest(
      UUID channelClaimIdentifier,
      UUID usersCustomerIdentifier,
      UUID channelClaimCustomerIdentifier,
      AccessRight accessRight)
      throws JsonProcessingException {
    return createAuthenticatedRequest(
        channelClaimIdentifier.toString(),
        usersCustomerIdentifier.toString(),
        channelClaimCustomerIdentifier.toString(),
        accessRight);
  }

  private InputStream createAuthenticatedRequest(
      String channelClaimIdentifier,
      String userAndChannelClaimCustomerIdentifier,
      AccessRight accessRight)
      throws JsonProcessingException {
    return createAuthenticatedRequest(
        channelClaimIdentifier, userAndChannelClaimCustomerIdentifier,
        userAndChannelClaimCustomerIdentifier, accessRight);
  }

  private InputStream createAuthenticatedRequest(
      String channelClaimIdentifier,
      String usersCustomerIdentifier,
      String channelClaimCustomerIdentifier,
      AccessRight accessRight)
      throws JsonProcessingException {
    ...
    return new HandlerRequestBuilder<Void>(dtoObjectMapper)
        ...
        .withPathParameters(
            Map.of(
                IDENTIFIER,
                channelClaimCustomerIdentifier,
                CHANNEL_CLAIM_IDENTIFIER,
