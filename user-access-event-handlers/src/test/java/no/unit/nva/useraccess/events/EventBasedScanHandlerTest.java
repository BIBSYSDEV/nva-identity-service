...
class EventBasedScanHandlerTest extends LocalIdentityService {
  ...
  @ParameterizedTest(
      name =
          "should send message to Event Bridge for next scan containing the key of the last "
              + "scanned object.Page size:{0}")
  @ValueSource(ints = {1, 5, 10, 50})
  void shouldSendEventForNextScanContainingTheKeyOfLastScannedObjectAsNextScanStartMarker(
      int pageSize) {
  ...
  private UserDto fetchLastScannedUserUsingTheEmittedEvent(
      ScanDatabaseRequestV2 emittedScanRequest) {
  ...
  private List<UserDto> scanAllUsersInDatabaseDirectly() {
    return localDynamo
        .scanPaginator(ScanRequest.builder().tableName(Constants.USERS_AND_ROLES_TABLE).build())
        .stream()
  ...
  private ScanDatabaseRequestV2 createSampleScanRequest(Integer pageSize) {
    return new ScanDatabaseRequestV2(
        IDENTITY_SERVICE_BATCH_SCAN_EVENT_TOPIC, pageSize, NO_START_MARKER);
  }

  @ParameterizedTest(
      name = "should scan all users in database and not enter an infinite loop. PageSize:{0}")
  ...
  private void performEventDrivenScanInWholeDatabase(InputStream firstEvent)
