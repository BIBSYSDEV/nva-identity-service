...
class CristinWireMockStubs {
  ...
  CristinWireMockStubs(
      String basicAuthorizationHeaderValue, URI cristinBaseUri, HttpHeaders defaultRequestHeaders) {
  ...
  void createStubsForPerson(String nin, CristinPerson cristinPerson) {
    var response =
        attempt(() -> JsonUtils.dtoObjectMapper.writeValueAsString(cristinPerson)).orElseThrow();
  ...
  void createStubForRedirectingInstitution(
      String originalInstitutionId, String redirectedInstitutionId) {
    stubWithDefaultRequestHeaders(
  ...
  void createPersonAlreadyExistsStub() {
    ...
    stubWithDefaultRequestHeaders(
        post(PERSONS)
            .withHeader(AUTHORIZATION_HEADER_NAME, equalTo(basicAuthorizationHeaderValue))
            .willReturn(
                aResponse().withStatus(HttpURLConnection.HTTP_BAD_REQUEST).withBody(errorBody)));
  }
  ...
  void createPersonConflictStub() {
    ...
    stubWithDefaultRequestHeaders(
        post(PERSONS)
            .withHeader(AUTHORIZATION_HEADER_NAME, equalTo(basicAuthorizationHeaderValue))
            .willReturn(
                aResponse().withStatus(HttpURLConnection.HTTP_CONFLICT).withBody(errorBody)));
  }
  ...
  String generateInstitutionUrl(String identifier) {
    return UriWrapper.fromUri(cristinBaseUri)
        .addChild("institutions", identifier)
        .getUri()
        .toString();
  }
