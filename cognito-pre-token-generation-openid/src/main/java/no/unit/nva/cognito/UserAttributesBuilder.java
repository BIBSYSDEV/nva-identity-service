...
public class UserAttributesBuilder {
  ...
  public UserAttributesBuilder withAuthenticationDetails(
  ...
  public UserAttributesBuilder withAccessRightsPersistedFormat(
  ...
  public List<AttributeType> build() {
    ...
    userAttributes.add(
        createAttribute(
            ACCESS_RIGHTS_CLAIM, String.join(ELEMENTS_DELIMITER, accessRightsPersistedFormat)));
    ...
    var allowedCustomersString =
        createAllowedCustomersString(
            userAcceptedTerms, customersForPerson, authenticationDetails.getFeideDomain());
    ...
    userAttributes.add(
        createAttribute(IMPERSONATED_BY_CLAIM, isNull(impersonatedBy) ? "" : impersonatedBy));
    userAttributes.add(createAttribute(CURRENT_TERMS, currentTerms.toString()));
    userAttributes.add(
        createAttribute(
            CUSTOMER_ACCEPTED_TERMS, nonNull(acceptedTerms) ? acceptedTerms.toString() : ""));

    if (currentCustomer != null && currentUser != null) {
      userAttributes.add(
          createAttribute(CURRENT_CUSTOMER_CLAIM, currentCustomer.getId().toString()));
      userAttributes.add(
          createAttribute(TOP_ORG_CRISTIN_ID, currentCustomer.getCristinId().toString()));
      userAttributes.add(createAttribute(NVA_USERNAME_CLAIM, currentUser.getUsername()));
      userAttributes.add(
          createAttribute(PERSON_AFFILIATION_CLAIM, currentUser.getAffiliation().toString()));
    } else {
    ...
    userAttributes.add(
        createAttribute(
            VIEWING_SCOPE_INCLUDED_CLAIM, uriSetToCommaSeparatedString(viewingScopeIncluded)));

    var viewingScopeExcluded = getViewingScope(ViewingScope::getExcludedUnits, currentUser);
    userAttributes.add(
        createAttribute(
            VIEWING_SCOPE_EXCLUDED_CLAIM, uriSetToCommaSeparatedString(viewingScopeExcluded)));

    return userAttributes;
  }

  private String createAllowedCustomersString(
      boolean userAcceptedTerms, Collection<CustomerDto> allowedCustomers, String feideDomain) {
    var result =
    ...
    return StringUtils.isNotBlank(result) && userAcceptedTerms ? result : EMPTY_CLAIM;
  }
  ...
  private String rolesForCustomer(
      List<UserDto> usersForPerson, CustomerDto customer, boolean userAcceptedTerms) {
    if (isNull(customer) || !userAcceptedTerms) {
      return EMPTY_STRING;
    }
    var rolesPerCustomerForPerson =
        usersForPerson.stream()
    ...
    return String.join(
        ELEMENTS_DELIMITER, rolesPerCustomerForPerson.stream().map(RoleName::getValue).toList());
  }
