...
class CreateControlledVocabularyTest extends CreateUpdateControlledVocabularySettingsTests {
  ...
  void handleRequestReturnsResponseWithContentTypeJsonLdWhenAcceptHeaderIsJsonLd()
    ...
    var result = sendRequestAcceptingJsonLd(existingIdentifier());
    assertThat(
        responseContentType(result.getResponse()),
  ...
  @Test
  void handleRequestReturnsUnsupportedTypeWhenAcceptedContentTypeIsNotSupported()
      throws IOException {
    var result = sendRequest(existingIdentifier(), UNSUPPORTED_MEDIA_TYPE);

    assertThat(
        result.getResponse().getStatusCode(), is(equalTo(HttpURLConnection.HTTP_UNSUPPORTED_TYPE)));
  ...
  @Test
  void
      handleRequestReturnsCreatedVocabularyListWhenUserWithAccessRightManageOwnAffiliationsCreatesVocabulary()
          throws IOException {
    var result =
