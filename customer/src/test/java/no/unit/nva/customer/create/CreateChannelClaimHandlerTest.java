...
class CreateChannelClaimHandlerTest extends LocalCustomerServiceDatabase {
  ...
  private HandlerRequestBuilder<ChannelClaimRequest> createDefaultRequestBuilder(
      UUID customerToClaim, ChannelClaimRequest channelClaim) throws JsonProcessingException {
  ...
  private InputStream createValidRequest() throws JsonProcessingException {
    return createDefaultRequestBuilder(
            existingCustomer.getIdentifier(), randomChannelClaimRequest())
        .withAccessRights(existingCustomer.getId(), MANAGE_CHANNEL_CLAIMS)
        .build();
  }

  private InputStream createRequestClaimingChannelAlreadyClaimedByCustomer()
  ...
  private InputStream createRequestClaimingChannelClaimedByAnotherCustomer()
    ...
    var existingClaimByAnotherCustomer =
        anotherCustomer.getChannelClaims().stream().findFirst().orElseThrow();
    return createDefaultRequestBuilder(
            existingCustomer.getIdentifier(), fromDto(existingClaimByAnotherCustomer))
        .withAccessRights(existingCustomer.getId(), MANAGE_CHANNEL_CLAIMS)
        .build();
  }
  ...
  private InputStream createRequestWithNonExistingCustomer() throws JsonProcessingException {
    ...
    var nonExistingCustomerId =
        UriWrapper.fromHost(randomString())
  ...
  private InputStream createRequestWithInvalidChannelUriInBody() throws JsonProcessingException {
    var channelClaimRequest =
        new ChannelClaimRequest(randomUri(), fromDto(randomChannelConstraintDto()));
