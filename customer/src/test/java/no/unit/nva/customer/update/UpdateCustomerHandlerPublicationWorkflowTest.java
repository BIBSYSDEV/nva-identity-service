...
public class UpdateCustomerHandlerPublicationWorkflowTest extends LocalCustomerServiceDatabase {
  ...
  void shouldUpdatePublicationWorkflowWhenAuthorized() throws IOException {
    ...
    assertThat(
        updateCustomer.getPublicationWorkflow(),
        is(not(equalTo(preExistingCustomer.getPublicationWorkflow()))));

    var request =
        updateRequestWithCorrectAuthorization(
            updateCustomer, getIdentifierPathParam(updateCustomer));
    var response = sendUpdateRequest(request, CustomerDto.class);

    assertThat(response.getStatusCode(), is(equalTo(HttpURLConnection.HTTP_OK)));
    assertThat(
        response.getBodyObject(CustomerDto.class).getPublicationWorkflow(),
  ...
  private InputStream updateRequestWithCorrectAuthorization(
      CustomerDto updateCustomer, Map<String, String> pathParameters)
      throws JsonProcessingException {
    return new HandlerRequestBuilder<CustomerDto>(dtoObjectMapper)
        .withPathParameters(pathParameters)
        .withCurrentCustomer(updateCustomer.getId())
        .withAccessRights(updateCustomer.getId(), AccessRight.MANAGE_OWN_AFFILIATION)
  ...
  private void createCustomerInLocalDb() throws IOException {
    ...
    createHandler.handleRequest(
        createCustomerInDbRequest(createCustomerBase), createOutputStream, new FakeContext());

    preExistingCustomer =
        GatewayResponse.fromOutputStream(createOutputStream, CustomerDto.class)
  ...
  private <T> GatewayResponse<T> sendUpdateRequest(InputStream request, Class<T> responseType)
      throws IOException {
  ...
  void shouldUpdatePublicationWorkflowWhenAuthorizedAsCognitoAdmin() throws IOException {
    ...
    var request =
        updateRequestWithCognitoAdminAuthorization(
            updateCustomer, getIdentifierPathParam(updateCustomer));
    var response = sendUpdateRequest(request, CustomerDto.class);
  ...
  private InputStream updateRequestWithCognitoAdminAuthorization(
      CustomerDto updateCustomer, Map<String, String> pathParameters)
      throws JsonProcessingException {
    return new HandlerRequestBuilder<CustomerDto>(dtoObjectMapper)
        .withPathParameters(pathParameters)
  ...
  @Test
  void shouldThrowForbiddenWhenTryingToUpdatePublicationWorkflowWhenNotAuthorized()
      throws IOException {
    createCustomerInLocalDb();
    var updateCustomer = changePublicationWorkflowFromPreExistingCustomer();
    var request =
        updateRequestWithoutCorrectAuthorization(
            updateCustomer, getIdentifierPathParam(updateCustomer));
    var response = sendUpdateRequest(request, CustomerDto.class);
  ...
  private InputStream updateRequestWithoutCorrectAuthorization(
      CustomerDto updateCustomer, Map<String, String> pathParameters)
      throws JsonProcessingException {
    return new HandlerRequestBuilder<CustomerDto>(dtoObjectMapper)
        .withPathParameters(pathParameters)
