...
public class SecretManagerDoiAgentDao implements DoiAgent {
  ...
  private URI agentIdToCustomerId(URI agentId) {
    return UriWrapper.fromUri(agentId).getParent().orElseThrow().getUri();
  }

  public static SecretManagerDoiAgentDao fromJson(String json) throws BadRequestException {
    return attempt(() -> JsonConfig.readValue(json, SecretManagerDoiAgentDao.class))
        .orElseThrow(
  ...
  public DoiAgentDto toDoiAgentDto() {
    return new DoiAgentDto(this).addPassword(getPassword()).addId(customerIdToAgentId(customerId));
  }
