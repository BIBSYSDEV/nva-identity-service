...
public class DefaultCustomerResourceUpdateEventsProducer
    implements CustomerResourceUpdateEventsProducer {

  private static final Logger logger =
      LoggerFactory.getLogger(DefaultCustomerResourceUpdateEventsProducer.class);
  private static final TableSchema<CustomerDao> CUSTOMER_DAO_TABLE_SCHEMA =
      TableSchema.fromBean(CustomerDao.class);

  private final AttributeValueConverter attributeValueConverter;

  public DefaultCustomerResourceUpdateEventsProducer(
  ...
  private List<ResourceUpdateEvent<ChannelClaim>> computeChannelClaimEvents(
      URI customerId,
      URI organizationId,
      Collection<ChannelClaimDto> oldClaims,
      Collection<ChannelClaimDto> newClaims) {
    var oldMap =
        oldClaims.stream().collect(Collectors.toMap(ChannelClaimDto::channel, Function.identity()));

    var newMap =
        newClaims.stream().collect(Collectors.toMap(ChannelClaimDto::channel, Function.identity()));
  ...
  private List<ResourceUpdateEvent<ChannelClaim>> computeModifiedEvents(
      URI customerId,
      URI organizationId,
  ...
  private static List<ResourceUpdateEvent<ChannelClaim>> computeRemovedEvents(
      URI customerId,
      URI organizationId,
      Map<URI, ChannelClaimDto> oldMap,
      Map<URI, ChannelClaimDto> newMap) {
    return oldMap.keySet().stream()
        ...
        .map(key -> removedChannelClaim(customerId, organizationId, oldMap.get(key)))
        .toList();
  }

  private static List<ResourceUpdateEvent<ChannelClaim>> computeAddedEvents(
      URI customerId,
      URI organizationId,
      Map<URI, ChannelClaimDto> newMap,
      Map<URI, ChannelClaimDto> oldMap) {
