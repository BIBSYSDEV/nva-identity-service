...
public class CreateUserHandler extends HandlerWithEventualConsistency<CreateUserRequest, UserDto> {
  ...
  private static final String BAD_GATEWAY_ERROR_MESSAGE =
  ...
  protected void validateRequest(
      CreateUserRequest createUserRequest, RequestInfo requestInfo, Context context)
  ...
  private UserDto createNewUser(CreateUserRequest input)
    ...
    LOGGER.info(
        "Attempting to create new user '{}' at customer '{}'",
        input.cristinIdentifier(),
        input.customerId());
    var person = fetchCristinPersonFromIdentifierOrNin(input).orElseThrow();

    var customersWithActiveAffiliations =
        fetchCustomersWithActiveAffiliations(person.getAffiliations());
    explainWhyUserCannotBeCreated(person, customersWithActiveAffiliations);

    var customersFilteredByInput =
        customersWithActiveAffiliations.stream().filter(customerByIdOrCristinId(input)).toList();

    if (customersFilteredByInput.isEmpty()) {
      LOGGER.error(
          "Person '{}' is affiliated with [{}], which does not include input customer '{}'",
          person.getIdentifier(),
          customersWithActiveAffiliations.stream()
              .map(c -> c.getCristinId() + ":" + c.getId())
    ...
    var customers =
        Collections.singleton(
            customersFilteredByInput.stream().collect(SingletonCollector.collect()));
  ...
  private void explainWhyUserCannotBeCreated(Person person, Set<CustomerDto> customers)
  ...
  private String noCustomersForActiveAffiliations(String personId) {
    return String.format(
        "Person %s has no active affiliations with an Institution that is an NVA customer",
        personId);
  }
  ...
  private Optional<UserDto> fetchExistingUser(CreateUserRequest input)
      throws ConflictException, BadGatewayException {
  ...
  private Optional<Person> fetchCristinPersonFromIdentifierOrNin(CreateUserRequest input)
    ...
    try {
      return Optional.ofNullable(input.cristinIdentifier())
          ...
          .orElseGet(
              () -> personRegistry.fetchPersonByNin(fromString(input.nationalIdentityNumber())));
    } catch (Exception e) {
  ...
  private void authorize(CreateUserRequest input, RequestInfo requestInfo)
    ...
    if (roles.contains(RoleName.APPLICATION_ADMIN)
  ...
  private boolean userIsInternalBackendOrHasManageOwnAffiliationAccess(RequestInfo requestInfo) {
    return requestInfo.clientIsInternalBackend() || requestInfo.userIsAuthorized(MANAGE_CUSTOMERS);
  }
}
