...
public class UserService extends DatabaseSubService {
  ...
  public UserDto getUser(UserDto queryObject) throws NotFoundException {
    return getUserAsOptional(queryObject)
        .orElseThrow(
            () -> new NotFoundException(USER_NOT_FOUND_MESSAGE + queryObject.getUsername()));
  }
  ...
  private UserDto attemptToFetchObject(UserDto queryObject) {
    UserDao userDao =
        attempt(() -> UserDao.fromUserDto(queryObject))
            .map(this::fetchItem)
  ...
  /**
   * List all users at all institutions. Uses an expansive scan-operation and should not be used
   * outside of migration-code
   */
  public List<UserDto> listAllUsers() {

    return this.table.scan(buildScanRequestForAllUsers()).stream()
        .map(Page::items)
  ...
  private static ScanEnhancedRequest buildScanRequestForAllUsers() {
    var expression =
        Expression.builder()
            ...
            .putExpressionValue(":type", AttributeValue.builder().s(TYPE_VALUE).build())
  ...
  public List<UserDto> listUsers(URI institutionId) {
    ...
    var users = result.stream().map(Page::items).flatMap(Collection::stream).toList();
    ...
    return QueryEnhancedRequest.builder()
        .queryConditional(
            QueryConditional.keyEqualTo(
                Key.builder().partitionValue(institution.toString()).build()))
  ...
  private List<RoleDb> currentRoles(UserDao currentUser) {
    return currentUser.getRolesNonNull().stream()
  ...
  public List<UserDto> getUsersByByCristinId(URI cristinPersonId) {
    var request =
        createQueryForSearchingInCristinCredentialsIndex(cristinPersonId, EMPTY_CRISTIN_ORG_ID);
  ...
  private QueryEnhancedRequest createQueryForSearchingInCristinCredentialsIndex(
      URI cristinPersonId, URI cristinOrgId) {
    var key =
        Optional.ofNullable(cristinOrgId)
            .map(
                orgId ->
                    Key.builder()
                        .partitionValue(cristinPersonId.toString())
                        .sortValue(orgId.toString()))
