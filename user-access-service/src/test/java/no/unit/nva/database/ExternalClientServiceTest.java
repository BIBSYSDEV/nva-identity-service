...
class ExternalClientServiceTest extends LocalIdentityService {
  ...
  void shouldInsertClientToDb()
      throws InvalidEntryInternalException, NotFoundException, URISyntaxException {
    var clientDto =
    ...
    ClientDto clientQuery = ClientDto.newBuilder().withClientId(CLIENT_ID).build();
  ...
  void throwsNotFoundExceptionWhenClientDoesNotExist() throws InvalidEntryInternalException {
    ClientDto clientQuery = ClientDto.newBuilder().withClientId(randomString()).build();
