...
public final class CustomerDataGenerator {
  ...
  public static ChannelConstraintDao randomChannelConstraintDao() {
    ...
    // return new ChannelConstraintDao(randomChannelPolicy(), randomChannelPolicy(),
    // randomScopes());
  }

  private static ChannelConstraintDao defaultChannelConstraintDao() {
    return new ChannelConstraintDao(
        ChannelConstraintPolicy.EVERYONE, ChannelConstraintPolicy.OWNER_ONLY, degreeScope());
  }
  ...
  public static ChannelConstraintDto randomChannelConstraintDto() {
    ...
    // Commented out until restriction of constraints are removed
    // return new ChannelConstraintDto(randomChannelPolicy(), randomChannelPolicy(),
    // randomScopes());
  }

  private static ChannelConstraintDto defaultChannelConstraintDto() {
    return new ChannelConstraintDto(
        ChannelConstraintPolicy.EVERYONE, ChannelConstraintPolicy.OWNER_ONLY, degreeScope());
  }
  ...
  public static VocabularyDao randomVocabularyDao() {
    return new VocabularyDao(randomString(), randomUri(), randomElement(VocabularyStatus.values()));
  }

  public static RightsRetentionStrategy randomRightsRetentionStrategy() {
    var elements =
        Arrays.stream(RightsRetentionStrategyType.values()).filter(f -> f.ordinal() > 0).toList();
    return new RightsRetentionStrategyDao(randomElement(elements), randomUri());
  }
