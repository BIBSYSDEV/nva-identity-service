...
public class UpdateUserHandlerTest extends HandlerTest {
  ...
  @DisplayName(
      "handleRequest() returns 202 (Accepted)  when path contains an existing user id, and the body"
          + " contains a valid UserDto and path id is the same as the body id ")
  @Test
  public void
      processInputReturnsAcceptedWhenPathAndBodyContainTheSameUserIdAndTheIdExistsAndBodyIsValid()
  ...
  private UserDto createUserUpdate(UserDto userDto) throws InvalidEntryInternalException {
    ...
    var someOtherRole = RoleDto.newBuilder().withRoleName(anotherRoleName).build();
    return userDto
        .copy()
  ...
  private <I, O> GatewayResponse<O> sendUpdateRequest(
      String userId, I userUpdate, Class<O> responseType) throws IOException {
    var updateUserHandler = new UpdateUserHandler(databaseService, new Environment());
    var customerId = randomUri();
    var input =
        new HandlerRequestBuilder<I>(dtoObjectMapper)
            .withBody(userUpdate)
  ...
  @DisplayName(
      "handleRequest() returns BadRequest  when path contains an Id that is different from the Id"
          + "of the input Object")
  @Test
  public void processInputReturnsBadRequestWhenPathContainsIdDifferentFromIdOfInputObject()
    ...
    assertThat(
        gatewayResponse.getBody(), containsString(INCONSISTENT_USERNAME_IN_PATH_AND_OBJECT_ERROR));
  }
  ...
  @DisplayName(
      "handleRequest() returns Location header with the URI to updated user when path contains an"
          + " existing user id, and the body contains a valid UserDto and path id is the same as"
          + " the body id ")
  @Test
  void
      handleRequestReturnsUpdatedUserWhenPathAndBodyContainTheSameUserIdAndTheIdExistsAndBodyIsValid()
          throws ApiGatewayException, IOException {
  ...
  @Test
  void handleRequestCanProcessUrlEncodedUsernames() throws ApiGatewayException, IOException {
  ...
  @DisplayName(
      "handleRequest() returns InternalServerError when handler is called without path parameter")
  @Test
  ...
  private GatewayResponse<Problem> sendUpdateRequestWithoutPathParameters(UserDto userUpdate)
    ...
    var updateUserHandler = new UpdateUserHandler(databaseService, new Environment());
    var input =
        new HandlerRequestBuilder<UserDto>(dtoObjectMapper)
            .withCurrentCustomer(customerId)
  ...
  @Test
  void shouldDenyAccessForUserWhenItDoesNotHaveAccessRights()
      throws IOException, ConflictException, NotFoundException {
    var userUpdate = createUserUpdateForElevatingRole();
    var gatewayResponse =
        sendUpdateRequestWithoutAccessRights(userUpdate.getUsername(), userUpdate, UserDto.class);
  ...
  private <I, O> GatewayResponse<O> sendUpdateRequestWithoutAccessRights(
      String userId, I userUpdate, Class<O> responseType) throws IOException {
    var updateUserHandler = new UpdateUserHandler(databaseService, new Environment());
    var input =
        new HandlerRequestBuilder<I>(dtoObjectMapper)
  ...
  void shouldDenyAccessWhenInstitutionAdminTriesToCreateAnAppAdmin()
    ...
    var userUpdate = createUserUpdateForElevatingRole();
    var gatewayResponse =
        sendUpdateRequestForInstitutionAdmin(userUpdate.getUsername(), userUpdate, UserDto.class);
    assertThat(gatewayResponse.getStatusCode(), is(equalTo(HttpStatus.SC_FORBIDDEN)));
  }

  private <I, O> GatewayResponse<O> sendUpdateRequestForInstitutionAdmin(
      String userId, I userUpdate, Class<O> responseType) throws IOException {
    var updateUserHandler = new UpdateUserHandler(databaseService, new Environment());
    var customerId = randomUri();
    var input =
  ...
  @Test
  void shouldAllowAccessWhenAppAdminTriesToCreateAnAppAdmin()
      throws IOException, ConflictException, NotFoundException {
    var userUpdate = createUserUpdateForElevatingRole();
    var gatewayResponse = sendUpdateRequest(userUpdate.getUsername(), userUpdate, UserDto.class);
    assertThat(gatewayResponse.getStatusCode(), is(equalTo(HttpStatus.SC_ACCEPTED)));
  ...
  private static RoleDto getRandomRole() {
    return RoleDto.newBuilder()
        .withRoleName(randomRoleNameButNot(RoleName.APPLICATION_ADMIN))
        .build();
  }

  private UserDto createUserUpdateWithRoles(UserDto userDto, List<RoleDto> roles) {
    return userDto.copy().withRoles(roles).withViewingScope(randomViewingScope()).build();
  }
}
