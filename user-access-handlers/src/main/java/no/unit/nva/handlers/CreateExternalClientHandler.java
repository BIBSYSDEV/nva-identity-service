...
public class CreateExternalClientHandler
    extends HandlerWithEventualConsistency<
        CreateExternalClientRequest, CreateExternalClientResponse> {
  ...
  private static final String EXTERNAL_USER_POOL_URL =
      new Environment().readEnv("EXTERNAL_USER_POOL_URL");
  ...
  public CreateExternalClientHandler(
      IdentityService databaseService, CognitoService cognitoService, Environment environment) {
  ...
  protected void validateRequest(
      CreateExternalClientRequest createExternalClientRequest,
  ...
  @Override
  protected CreateExternalClientResponse processInput(
      CreateExternalClientRequest input, RequestInfo requestInfo, Context context)
      throws ApiGatewayException {

    var cognitoResponse =
        this.cognitoService.createUserPoolClient(input.getClientName(), input.getScopes());
    var clientDto =
  ...
  private CreateExternalClientResponse formatResponse(
      URI customer, UserPoolClientType userPoolClientType) {
  ...
  protected Integer getSuccessStatusCode(
      CreateExternalClientRequest input, CreateExternalClientResponse output) {
  ...
  private void validateRequest(CreateExternalClientRequest input) throws BadRequestException {
    ...
    if (!issues.isEmpty()) {
      throw new BadRequestException("Issues validating request: " + String.join(", ", issues));
    }
  }
