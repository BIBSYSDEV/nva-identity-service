...
class ViewingScopeTest {
  ...
  public static Stream<ViewingScope> viewingScopeProvider() throws BadRequestException {
    return Stream.of(
        new ViewingScope(Set.of(randomCristinOrgId()), null),
  ...
  void shouldThrowBadRequestExceptionWhenParsingFails() {
    ...
    var exception =
        assertThrows(BadRequestException.class, () -> ViewingScope.fromJson(invalidJson));
    assertThat(exception.getMessage(), containsString(invalidJson));
  }

  @Test
  void shouldDeserializeNullViewingScopeToViewingScopeWithEmptyList()
