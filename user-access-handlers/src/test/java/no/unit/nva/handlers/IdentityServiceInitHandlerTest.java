...
class IdentityServiceInitHandlerTest {
  ...
  private static final List<AccessRight> ACCESS_RIGHTS =
      List.of(MANAGE_DOI, MANAGE_PUBLISHING_REQUESTS);
  private static final RoleSource ROLE_SOURCE =
      () ->
          List.of(
              RoleDto.newBuilder()
  ...
  private void initializeIdentityService() {
    ...
    this.identityService =
        spy(new IdentityServiceImpl(this.identityServiceLocalDb.getDynamoDbClient()));
  }
  ...
  void shouldCreateTheDefaultRolesForTheService() throws IOException {
    ...
    var handler =
        new IdentityServiceInitHandler(
            identityService, customerService, ROLE_SOURCE, new Environment());
  ...
  void shouldCreateSiktBackendClientDBRow() throws NotFoundException, IOException {
    ...
    handler.handleRequest(createRequest(), output, context);
    var client =
        identityService.getClient(ClientDto.newBuilder().withClientId(BACKEND_CLIENT_ID).build());
