...
public class UpdateUserHandler extends HandlerAccessingUser<UserDto, Void> {
    ...
    private void authorizeRequest(UserDto input, RequestInfo requestInfo) throws ForbiddenException {
        if (!isAuthorized(input, requestInfo)) {
            throw new ForbiddenException();
        }
    }

    private boolean isAuthorized(UserDto input, RequestInfo requestInfo) {
        return requestInfo.clientIsInternalBackend()
               || isAuthorizedUser(input, requestInfo);
    }

    private boolean isAuthorizedUser(UserDto input, RequestInfo requestInfo) {
        var roles = input.getRoles().stream().map(RoleDto::getRoleName).collect(Collectors.toSet());
        if (roles.contains(APP_ADMIN_ROLE_NAME)) {
            return requestInfo.userIsAuthorized(MANAGE_CUSTOMERS);
        }

        return requestInfo.userIsAuthorized(AccessRight.MANAGE_OWN_AFFILIATION)
