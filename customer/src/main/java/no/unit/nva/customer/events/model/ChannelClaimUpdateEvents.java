...
public final class ChannelClaimUpdateEvents {
  ...
  public static ResourceUpdateEvent<ChannelClaim> addedChannelClaim(
      URI customerId, URI organizationId, ChannelClaimDto claim) {
    return channelClaimEvent(customerId, organizationId, ADDED, claim);
  }

  public static ResourceUpdateEvent<ChannelClaim> removedChannelClaim(
      URI customerId, URI organizationId, ChannelClaimDto claim) {
    return channelClaimEvent(customerId, organizationId, REMOVED, claim);
  }

  public static ResourceUpdateEvent<ChannelClaim> updatedChannelClaim(
      URI customerId, URI organizationId, ChannelClaimDto claim) {
    return channelClaimEvent(customerId, organizationId, UPDATED, claim);
  }

  private static ResourceUpdateEvent<ChannelClaim> channelClaimEvent(
      URI customerId, URI organizationId, Action action, ChannelClaimDto claim) {
    var constraint =
        new Constraints(
            EnumSet.copyOf(claim.constraint().scope()),
            claim.constraint().publishingPolicy(),
            claim.constraint().editingPolicy());
    var channelClaim =
        new ChannelClaim(
            ChannelClaimIdProducer.channelClaimId(claim),
            claim.channel(),
            customerId,
            organizationId,
            constraint);
