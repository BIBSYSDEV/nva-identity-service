...
class ChannelClaimValidatorTest {
  ...
  void shouldThrowBadRequestExceptionWhenChannelIsMissing() {
    ...
    assertThrows(
        ...
        () -> ChannelClaimValidator.validate(channelClaimWithoutChannel));
  }

  @Test
  void shouldThrowBadRequestExceptionWhenChannelIsInvalid() {
    var channelClaimWithInvalidChannel =
        new ChannelClaimDto(randomUri(), randomChannelConstraintDto());
    assertThrows(
        BadRequestException.class,
        () -> ChannelClaimValidator.validate(channelClaimWithInvalidChannel));
  }

  @Test
  void shouldThrowBadRequestExceptionWhenChannelHasTrailingYear() {
    var channelWithTrailingYear =
        UriWrapper.fromUri(randomChannel()).addChild(RANDOM_YEAR).getUri();
    var channelClaimWithInvalidChannel =
        new ChannelClaimDto(channelWithTrailingYear, randomChannelConstraintDto());
    assertThrows(
        BadRequestException.class,
        () -> ChannelClaimValidator.validate(channelClaimWithInvalidChannel));
  }
  ...
  void shouldThrowBadRequestExceptionWhenLastPathElementOfChannelIsNotUuid() {
    var channelWithRandomLastPathElement =
        UriWrapper.fromUri(randomChannel())
            .replacePathElementByIndexFromEnd(ZERO, EMPTY_STRING)
            .getUri();
    var channelClaimWithInvalidChannel =
        new ChannelClaimDto(channelWithRandomLastPathElement, randomChannelConstraintDto());
    assertThrows(
        BadRequestException.class,
        () -> ChannelClaimValidator.validate(channelClaimWithInvalidChannel));
  }
  ...
  void shouldThrowBadRequestExceptionWhenNoConstraintIsProvided() {
    ...
    assertThrows(
        BadRequestException.class,
        () -> ChannelClaimValidator.validate(channelClaimWithoutConstraint));
  }
  ...
  void shouldThrowBadRequestExceptionWhenNoScopeIsProvided() {
    ...
    assertThrows(
        BadRequestException.class, () -> ChannelClaimValidator.validate(channelClaimWithoutScope));
  }
  ...
  void shouldThrowBadRequestExceptionWhenPublishingPolicyIsOwnerOnly() {
    ...
    var channelClaimWithNotDefaultConstraint =
        new ChannelClaimDto(randomChannel(), nonDefaultConstraint);
    assertThrows(
        BadRequestException.class,
  ...
  void shouldThrowBadRequestExceptionWhenWhenAllScopesAreNotDegree() {
    var nonDefaultConstraint =
        new ChannelConstraintDto(EVERYONE, OWNER_ONLY, List.of(ACADEMIC_ARTICLE, DEGREE_MASTER));
    var channelClaimWithNotDefaultConstraint =
        new ChannelClaimDto(randomChannel(), nonDefaultConstraint);
    assertThrows(
        BadRequestException.class,
