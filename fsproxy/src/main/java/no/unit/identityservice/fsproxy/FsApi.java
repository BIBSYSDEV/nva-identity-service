...
public class FsApi {
  ...
  public static final String FETCH_FS_USER_EXCEPTION_MESSAGE =
      "Not possible to fetch fs id-number to user " + "with following national number: ";
  public static final String COURSES_FETCHED_SUCCESSFULLY =
      "Courses fetched successfully to user with" + " id-number ";
  ...
  public List<FsCourse> fetchCoursesForPerson(NationalIdentityNumber nin)
    ...
    return Stream.concat(coursesIfStaff.stream(), coursesIfStudent.stream())
  ...
  public FsIdNumber fetchFsId(NationalIdentityNumber nin) throws IOException, InterruptedException {
    ...
    return fsIdSearchResult
        .getSearchResult()
        .orElseThrow(
            () -> new UserPrincipalNotFoundException(FETCH_FS_USER_EXCEPTION_MESSAGE + nin))
        .getFsPerson()
        .getFsIdNumber();
  }

  public List<FsCourse> fetchCoursesToStudent(FsIdNumber fsIdNumber)
      throws IOException, InterruptedException {
    ...
    return fsCoursesSearchResult.getItems() == null
        ? Collections.emptyList()
        : fsCoursesSearchResult.getItems().stream()
  ...
  private List<FsCourse> fetchCoursesForStaffPerson(FsIdNumber fsId)
      throws IOException, InterruptedException {
  ...
  private List<FsCourse> fetchCourseDetails(Stream<FsUriToCourseActivity> coursesUri) {
    return coursesUri
        .map(attempt(this::fetchCourseToStaffPersonGivenUriToCourse))
        .map(Try::orElseThrow)
        .collect(Collectors.toList());
  }

  private Stream<FsUriToCourseActivity> fetchAllCourseUrisBasedOnRolesOfStaffPerson(
      List<FsRoleToStaffPerson> roles) {
    if (roles == null) {
      return Stream.empty();
    }
    return roles.stream()
        .map(attempt(this::fetchCourseActivityUriToGivenRole))
        .map(Try::orElseThrow);
  }
