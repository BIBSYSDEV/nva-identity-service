...
public class SingleTableCrudService<T extends DataAccessClass<T>> implements DataAccessService<T> {
  ...
  public void persist(T newItem) {
    ...
    optionalFetchBy(newItem)
        .ifPresentOrElse(
            oldItem -> table.putItem(oldItem.merge(newItem)), () -> table.putItem(newItem));
  }
  ...
  private Optional<T> optionalFetchBy(T item) {
    var key = Key.builder().partitionValue(item.id().toString()).sortValue(item.type()).build();
