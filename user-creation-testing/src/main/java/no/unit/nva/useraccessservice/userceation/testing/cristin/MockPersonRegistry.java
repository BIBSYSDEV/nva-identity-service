...
public class MockPersonRegistry {
  ...
  public MockPersonRegistry(
      String username, String password, URI cristinBaseUri, HttpHeaders defaultRequestHeaders) {
    ...
    this.wireMockStubs =
        new CristinWireMockStubs(
            basicAuthorizationHeaderValue, cristinBaseUri, defaultRequestHeaders);
  }
  ...
  public MockedPersonData personWithOneActiveAndOneInactiveEmploymentInDifferentInstitutions() {
    ...
    var affiliations =
        List.of(createCristinAffiliation(ACTIVE), createCristinAffiliation(INACTIVE));
  ...
  public MockedPersonData personWithTwoActiveEmploymentsInDifferentInstitutions() {
    ...
    var affiliations = List.of(createCristinAffiliation(ACTIVE), createCristinAffiliation(ACTIVE));
  ...
  public MockedPersonData personWithActiveAffiliationAtRedirectingInstitution() {
    ...
    var institution =
        new CristinAffiliationInstitution(
            originalInstitutionId, wireMockStubs.generateInstitutionUrl(originalInstitutionId));
    var unit = createAffiliationUnit();
    var affiliation = new CristinAffiliation(institution, unit, ACTIVE);

    var person =
        new CristinPerson(cristinId, randomString(), randomString(), List.of(affiliation), null);

    ninToPeople.put(nin, person);
    cristinIdToPeople.put(person.getId(), person);
    wireMockStubs.createStubsForPerson(nin, person);
    wireMockStubs.createStubForRedirectingInstitution(
        originalInstitutionId, redirectedCorrespondingUnitId);
  ...
  private void updateBuffersAndStubs(
      String nin, CristinPerson cristinPerson, List<String> institutionIds) {
    if (nonNull(nin)) {
  ...
  private void createStubForInstitution(String institutionIdentifier) {
    ...
    this.cristinInstitutionIdToUnitUriMap.put(
        institutionIdentifier, nvaScopedOrganizationCristinId(institutionUnitIdentifier));
  ...
  private CristinAffiliation createCristinAffiliation(
      CristinAffiliationInstitution institution, boolean active) {
    var unit = createAffiliationUnit();
    return new CristinAffiliation(institution, unit, active);
  }
  ...
  private CristinAffiliationInstitution createAffiliationInstitution() {
    ...
    return new CristinAffiliationInstitution(
        identifier, wireMockStubs.generateInstitutionUrl(identifier));
  }
  ...
  private void createPersonWithAffiliations(
      String nin, String cristinId, List<CristinAffiliation> cristinAffiliations) {
    var person =
        new CristinPerson(cristinId, randomString(), randomString(), cristinAffiliations, null);

    var institutions =
        cristinAffiliations.stream()
  ...
  private Set<URI> getInstitutionUnitCristinUris(
      String nin, Predicate<CristinAffiliation> predicate) {
    var person = getPerson(nin);
    return person.getAffiliations().stream()
        .filter(predicate)
        .map(CristinAffiliation::getInstitution)
