...
class ListByInstitutionHandlerTest extends HandlerTest {
  ...
  private <T> GatewayResponse<T> sendRequestToHandler(
      InputStream validRequest, Class<T> responseBodyType) throws IOException {
  ...
  private InputStream createListRequest(URI institutionId) throws JsonProcessingException {
    Map<String, String> queryParams =
        Map.of(INSTITUTION_ID_QUERY_PARAMETER, institutionId.toString());
    return new HandlerRequestBuilder<>(dtoObjectMapper).withQueryParameters(queryParams).build();
  }
  ...
  private UserList parseResponseBody(GatewayResponse<UserList> response)
  ...
  private void assertThatListsAreEquivalent(UserList expectedUsers, UserList actualUsers) {
    assertThat(
        ...
        containsInAnyOrder(expectedUsers.getUsers().toArray(UserDto[]::new)));
  ...
  void handleRequestReturnsListOfUsersGivenAnInstitutionAndSingleRole()
    ...
    var expectedUser =
        insertUserOfSameInstitution(SOME_OTHER_USERNAME, randomRoleNameButNot(name))
            .getUsers()
            .get(0);
  ...
  private InputStream createListWithFilterRequest(URI institutionId, List<RoleName> roles)
      throws JsonProcessingException {
    var queryParams = Map.of(INSTITUTION_ID_QUERY_PARAMETER, institutionId.toString());
  ...
  void handleRequestReturnsListOfUsersGivenAnInstitutionAndMultipleRoles()
    ...
    var user1 =
        insertSampleUserToDatabase(randomString(), DEFAULT_INSTITUTION, RoleName.SUPPORT_CURATOR);
    var user2 =
        insertSampleUserToDatabase(randomString(), DEFAULT_INSTITUTION, RoleName.SUPPORT_CURATOR);
    var user3 =
        insertSampleUserToDatabase(randomString(), DEFAULT_INSTITUTION, RoleName.DOI_CURATOR);
    var rolesOfFirstTwoUsers =
        List.of(
  ...
  private InputStream createListWithFilterUserNameRequest(URI institutionId, String username)
      throws JsonProcessingException {
    var queryParams =
        Map.of(INSTITUTION_ID_QUERY_PARAMETER, institutionId.toString(), NAME, username);
  ...
  private UserList unexpectedUsers(UserList insertedUsers, UserList expectedUsers) {
    ...
    // use ArrayList explicitly because the List returned by the asList() method does not support
    // removeAll
  ...
  private void assertThatActualResultDoesNotContainUnexpectedEntities(
      UserList actualUsers, UserList unexpectedUsers) {
