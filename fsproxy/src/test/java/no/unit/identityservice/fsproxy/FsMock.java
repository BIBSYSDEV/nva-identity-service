...
public class FsMock {
    ...
    public Nin createExistingPersonWithoutEmployment() {
        ...
        var personEntry =
            new FsPerson(randomFsIsNumber(), EMPTY_FAG_PERSON, person.getBirthDate(), person.getPersonalNumber());
    ...
    private void addResponseWhenGettingById(FsPerson fsPerson) {
        server.stubFor(get(urlPathEqualTo("/" + PERSON_PATH + "/" + fsPerson.getFsIdNumber().toString()))
            .willReturn(ok().withHeader("Content-Type", "application/json")
                .withBody(fsPerson.toString())));
    }
    
    private void addResponseWhenForPersonNotRegisteredInFs(Nin nin) {
        server.stubFor(get(urlPathEqualTo("/" + PERSON_PATH))
