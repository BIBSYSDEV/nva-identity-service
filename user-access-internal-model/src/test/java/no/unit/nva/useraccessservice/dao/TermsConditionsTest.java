...
class TermsConditionsTest {
  ...
  void shouldMakeRoundTripWithoutLossOfInformation() throws JsonProcessingException {
    ...
    var regeneratedObject =
        JsonUtils.dtoObjectMapper.readValue(objectAsString, TermsConditionsResponse.class);
    assertThat(randomLicenseInfo, is(equalTo(regeneratedObject)));
  }

  private TermsConditionsResponse randomTermsConditionsResponse() {
    return new TermsConditionsResponse.Builder().withTermsConditionsUri(randomUri()).build();
  }

  @Test
  void shouldMakeRoundTripWithoutLossOfInformationWhenLicenseInfoIsCreatedFromDao()
    ...
    var regeneratedObject =
        JsonUtils.dtoObjectMapper.readValue(objectAsString, TermsConditions.class);
  ...
  void shouldMakeRoundTripWithoutLossOfInformationWhenLicenseInfoIsCreatedFromDaoAndBack()
    ...
    var randomLicenseInfoDao = randomTermsConditions().build();
    var licenseInfo =
        new TermsConditionsResponse.Builder()
  ...
  void shouldMergeTermsConditionsSuccessfully() {
    ...
    assertThat(
        mergedTermsConditions.termsConditionsUri(),
        is(equalTo(secondTermsConditions.termsConditionsUri())));
  }

  @Test
  void shouldFetchTermsConditions() throws NotFoundException {
    var firstTerms = randomTermsConditions().build();
    when(mockedService.fetch(any())).thenReturn(firstTerms);
  ...
  @Test
  void shouldReturnEmptyOptionalWhenFetchingTermsConditionsThrowsNotFoundException()
      throws NotFoundException {
    when(mockedService.fetch(any())).thenThrow(new NotFoundException(""));
    var fetchedTermsConditions =
        Optional.ofNullable(new TermsConditions.Builder().build())
            .flatMap(
                termsConditions -> {
                  try {
