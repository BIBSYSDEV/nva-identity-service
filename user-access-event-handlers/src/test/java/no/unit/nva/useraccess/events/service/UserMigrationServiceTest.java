...
class UserMigrationServiceTest {
  ...
  private static final URI SAMPLE_ORG_ID =
      URI.create("https://localhost/cristin/organization/123.0.0.0");
  ...
  void shouldReturnUserWithDefaultViewingScope() throws Exception {
    ...
    var actualUser =
        userMigrationService.migrateUser(user.copy().build(), RESET_VIEWING_SCOPE_ACTION);
  ...
  private CustomerDto createSampleCustomer() {
    return CustomerDto.builder().withCristinId(SAMPLE_ORG_ID).build();
  }
