...
class ClientDaoTest {
  ...
  public void init() {
    ...
    sampleClient =
        ClientDao.newBuilder().withClientId(SOME_CLIENT_ID).withCustomer(SOME_ORG).build();
  }
  ...
  void builderShouldSetTheHashKeyBasedOnClientId() {
    ...
    String expectedHashKey =
        String.join(ClientDao.FIELD_DELIMITER, ClientDao.TYPE_VALUE, SOME_CLIENT_ID);
  ...
  void setTypeShouldNotAcceptWrongTypeValues() {
    ...
    assertThat(
        ...
        allOf(containsString(illegalType), containsString(ClientDao.TYPE_VALUE)));
  }
  ...
  private ClientDao randomClientDb() {
    ClientDao randomClient =
        ClientDao.newBuilder()
