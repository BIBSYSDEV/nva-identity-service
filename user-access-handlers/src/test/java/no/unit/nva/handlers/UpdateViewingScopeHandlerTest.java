...
public class UpdateViewingScopeHandlerTest extends HandlerTest {
  ...
  private InputStream createUpdateViewingScopeRequest(
      UserDto sampleUser, ViewingScope expectedViewingScope) throws JsonProcessingException {
  ...
  private <I> GatewayResponse<I> sendRequest(InputStream request, Class<I> responseType)
      throws IOException {
  ...
  private InputStream createInvalidUpdateViewingScopeRequest(UserDto objectThatIsNotViewingScope)
    ...
    var jsonMap =
        attempt(() -> JsonConfig.writeValueAsString(objectThatIsNotViewingScope))
    ...
    return new HandlerRequestBuilder<Map<String, ?>>(dtoObjectMapper)
        .withBody(jsonMap)
        .withPathParameters(
            Map.of(USERNAME_PATH_PARAMETER, objectThatIsNotViewingScope.getUsername()))
        .build();
  }

  @Test
  void shouldContainContentTypeHeaderWithValueJson() throws ConflictException, IOException {
    var sampleUser = addSampleUserToDb();
    var request = createUpdateViewingScopeRequest(sampleUser, randomViewingScope());
    var response = sendRequest(request, Problem.class);
  ...
  @Test
  void shouldReturnBadRequestWheRequestBodyIsInValid()
      throws InvalidEntryInternalException, IOException {
    var request =
        new HandlerRequestBuilder<String>(dtoObjectMapper).withBody(randomString()).build();
    var response = sendRequest(request, Problem.class);
